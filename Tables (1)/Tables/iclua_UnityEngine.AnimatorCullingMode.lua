---@meta
---@class UnityEngine.AnimatorCullingMode
UnityEngine.AnimatorCullingMode = {
---函数名：Equals
---@param p1 UnityEngine.AnimatorCullingMode
---@param p2 System.Object
---返回值：Boolean
---@return Boolean
Equals = function(p1,p2) end,

---函数名：GetHashCode
---@param p1 UnityEngine.AnimatorCullingMode
---返回值：Int32
---@return Int32
GetHashCode = function(p1) end,

---函数名：ToString
---@param p1 UnityEngine.AnimatorCullingMode
---返回值：String
---@return String
ToString = function(p1) end,

---函数名：ToString
---@param p1 UnityEngine.AnimatorCullingMode
---@param p2 System.String
---@param p3 System.IFormatProvider
---返回值：String
---@return String
ToString = function(p1,p2,p3) end,

---函数名：CompareTo
---@param p1 UnityEngine.AnimatorCullingMode
---@param p2 System.Object
---返回值：Int32
---@return Int32
CompareTo = function(p1,p2) end,

---函数名：ToString
---@param p1 UnityEngine.AnimatorCullingMode
---@param p2 System.String
---返回值：String
---@return String
ToString = function(p1,p2) end,

---函数名：ToString
---@param p1 UnityEngine.AnimatorCullingMode
---@param p2 System.IFormatProvider
---返回值：String
---@return String
ToString = function(p1,p2) end,

---函数名：HasFlag
---@param p1 UnityEngine.AnimatorCullingMode
---@param p2 System.Enum
---返回值：Boolean
---@return Boolean
HasFlag = function(p1,p2) end,

---函数名：GetTypeCode
---@param p1 UnityEngine.AnimatorCullingMode
---返回值：TypeCode
---@return TypeCode
GetTypeCode = function(p1) end,

---函数名：GetType
---@param p1 UnityEngine.AnimatorCullingMode
---返回值：Type
---@return Type
GetType = function(p1) end,

---函数名：value__
---@param p1 UnityEngine.AnimatorCullingMode
---返回值：Int32
---@return Int32
value__ = function(p1) end,

---函数名：AlwaysAnimate
---返回值：AnimatorCullingMode
---@return AnimatorCullingMode
AlwaysAnimate = function() end,

---函数名：CullUpdateTransforms
---返回值：AnimatorCullingMode
---@return AnimatorCullingMode
CullUpdateTransforms = function() end,

---函数名：CullCompletely
---返回值：AnimatorCullingMode
---@return AnimatorCullingMode
CullCompletely = function() end,

---函数名：BasedOnRenderers
---返回值：AnimatorCullingMode
---@return AnimatorCullingMode
BasedOnRenderers = function() end,

}
