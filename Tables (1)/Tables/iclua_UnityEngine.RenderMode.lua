---@meta
---@class UnityEngine.RenderMode
UnityEngine.RenderMode = {
---函数名：Equals
---@param p1 UnityEngine.RenderMode
---@param p2 System.Object
---返回值：Boolean
---@return Boolean
Equals = function(p1,p2) end,

---函数名：GetHashCode
---@param p1 UnityEngine.RenderMode
---返回值：Int32
---@return Int32
GetHashCode = function(p1) end,

---函数名：ToString
---@param p1 UnityEngine.RenderMode
---返回值：String
---@return String
ToString = function(p1) end,

---函数名：ToString
---@param p1 UnityEngine.RenderMode
---@param p2 System.String
---@param p3 System.IFormatProvider
---返回值：String
---@return String
ToString = function(p1,p2,p3) end,

---函数名：CompareTo
---@param p1 UnityEngine.RenderMode
---@param p2 System.Object
---返回值：Int32
---@return Int32
CompareTo = function(p1,p2) end,

---函数名：ToString
---@param p1 UnityEngine.RenderMode
---@param p2 System.String
---返回值：String
---@return String
ToString = function(p1,p2) end,

---函数名：ToString
---@param p1 UnityEngine.RenderMode
---@param p2 System.IFormatProvider
---返回值：String
---@return String
ToString = function(p1,p2) end,

---函数名：HasFlag
---@param p1 UnityEngine.RenderMode
---@param p2 System.Enum
---返回值：Boolean
---@return Boolean
HasFlag = function(p1,p2) end,

---函数名：GetTypeCode
---@param p1 UnityEngine.RenderMode
---返回值：TypeCode
---@return TypeCode
GetTypeCode = function(p1) end,

---函数名：GetType
---@param p1 UnityEngine.RenderMode
---返回值：Type
---@return Type
GetType = function(p1) end,

---函数名：value__
---@param p1 UnityEngine.RenderMode
---返回值：Int32
---@return Int32
value__ = function(p1) end,

---函数名：ScreenSpaceOverlay
---返回值：RenderMode
---@return RenderMode
ScreenSpaceOverlay = function() end,

---函数名：ScreenSpaceCamera
---返回值：RenderMode
---@return RenderMode
ScreenSpaceCamera = function() end,

---函数名：WorldSpace
---返回值：RenderMode
---@return RenderMode
WorldSpace = function() end,

}
