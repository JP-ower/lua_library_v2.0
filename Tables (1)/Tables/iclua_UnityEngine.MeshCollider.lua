---@meta
---@class UnityEngine.MeshCollider
UnityEngine.MeshCollider = {
---函数名：get_sharedMesh
---@param p1 UnityEngine.MeshCollider
---返回值：Mesh
---@return Mesh
sharedMesh = function(p1) end,

---函数名：set_sharedMesh
---@param p1 UnityEngine.MeshCollider
sharedMesh = function(p1) end,

---函数名：get_convex
---@param p1 UnityEngine.MeshCollider
---返回值：Boolean
---@return Boolean
convex = function(p1) end,

---函数名：set_convex
---@param p1 UnityEngine.MeshCollider
convex = function(p1) end,

---函数名：get_inflateMesh
---@param p1 UnityEngine.MeshCollider
---返回值：Boolean
---@return Boolean
inflateMesh = function(p1) end,

---函数名：set_inflateMesh
---@param p1 UnityEngine.MeshCollider
inflateMesh = function(p1) end,

---函数名：get_cookingOptions
---@param p1 UnityEngine.MeshCollider
---返回值：MeshColliderCookingOptions
---@return MeshColliderCookingOptions
cookingOptions = function(p1) end,

---函数名：set_cookingOptions
---@param p1 UnityEngine.MeshCollider
cookingOptions = function(p1) end,

---函数名：get_skinWidth
---@param p1 UnityEngine.MeshCollider
---返回值：Single
---@return Single
skinWidth = function(p1) end,

---函数名：set_skinWidth
---@param p1 UnityEngine.MeshCollider
skinWidth = function(p1) end,

---函数名：get_smoothSphereCollisions
---@param p1 UnityEngine.MeshCollider
---返回值：Boolean
---@return Boolean
smoothSphereCollisions = function(p1) end,

---函数名：set_smoothSphereCollisions
---@param p1 UnityEngine.MeshCollider
smoothSphereCollisions = function(p1) end,

---函数名：get_enabled
---@param p1 UnityEngine.MeshCollider
---返回值：Boolean
---@return Boolean
enabled = function(p1) end,

---函数名：set_enabled
---@param p1 UnityEngine.MeshCollider
enabled = function(p1) end,

---函数名：get_attachedRigidbody
---@param p1 UnityEngine.MeshCollider
---返回值：Rigidbody
---@return Rigidbody
attachedRigidbody = function(p1) end,

---函数名：get_attachedArticulationBody
---@param p1 UnityEngine.MeshCollider
---返回值：ArticulationBody
---@return ArticulationBody
attachedArticulationBody = function(p1) end,

---函数名：get_isTrigger
---@param p1 UnityEngine.MeshCollider
---返回值：Boolean
---@return Boolean
isTrigger = function(p1) end,

---函数名：set_isTrigger
---@param p1 UnityEngine.MeshCollider
isTrigger = function(p1) end,

---函数名：get_contactOffset
---@param p1 UnityEngine.MeshCollider
---返回值：Single
---@return Single
contactOffset = function(p1) end,

---函数名：set_contactOffset
---@param p1 UnityEngine.MeshCollider
contactOffset = function(p1) end,

---函数名：ClosestPoint
---@param p1 UnityEngine.MeshCollider
---@param p2 UnityEngine.Vector3
---返回值：Vector3
---@return Vector3
ClosestPoint = function(p1,p2) end,

---函数名：get_bounds
---@param p1 UnityEngine.MeshCollider
---返回值：Bounds
---@return Bounds
bounds = function(p1) end,

---函数名：get_sharedMaterial
---@param p1 UnityEngine.MeshCollider
---返回值：PhysicMaterial
---@return PhysicMaterial
sharedMaterial = function(p1) end,

---函数名：set_sharedMaterial
---@param p1 UnityEngine.MeshCollider
sharedMaterial = function(p1) end,

---函数名：get_material
---@param p1 UnityEngine.MeshCollider
---返回值：PhysicMaterial
---@return PhysicMaterial
material = function(p1) end,

---函数名：set_material
---@param p1 UnityEngine.MeshCollider
material = function(p1) end,

---函数名：Raycast
---@param p1 UnityEngine.MeshCollider
---@param p2 UnityEngine.Ray
---@param p3 UnityEngine.RaycastHit&
---@param p4 System.Single
---返回值：Boolean
---@return Boolean
Raycast = function(p1,p2,p3,p4) end,

---函数名：ClosestPointOnBounds
---@param p1 UnityEngine.MeshCollider
---@param p2 UnityEngine.Vector3
---返回值：Vector3
---@return Vector3
ClosestPointOnBounds = function(p1,p2) end,

---函数名：get_transform
---@param p1 UnityEngine.MeshCollider
---返回值：Transform
---@return Transform
transform = function(p1) end,

---函数名：get_gameObject
---@param p1 UnityEngine.MeshCollider
---返回值：GameObject
---@return GameObject
gameObject = function(p1) end,

---函数名：GetComponent
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---返回值：Component
---@return Component
GetComponent = function(p1,p2) end,

---函数名：GetComponent
---@param p1 UnityEngine.MeshCollider
---返回值：T
---@return T
GetComponent = function(p1) end,

---函数名：TryGetComponent
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---@param p3 UnityEngine.Component&
---返回值：Boolean
---@return Boolean
TryGetComponent = function(p1,p2,p3) end,

---函数名：TryGetComponent
---@param p1 UnityEngine.MeshCollider
---返回值：Boolean
---@return Boolean
TryGetComponent = function(p1) end,

---函数名：GetComponent
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---返回值：Component
---@return Component
GetComponent = function(p1,p2) end,

---函数名：GetComponentInChildren
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---@param p3 System.Boolean
---返回值：Component
---@return Component
GetComponentInChildren = function(p1,p2,p3) end,

---函数名：GetComponentInChildren
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---返回值：Component
---@return Component
GetComponentInChildren = function(p1,p2) end,

---函数名：GetComponentInChildren
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Boolean
---返回值：T
---@return T
GetComponentInChildren = function(p1,p2) end,

---函数名：GetComponentInChildren
---@param p1 UnityEngine.MeshCollider
---返回值：T
---@return T
GetComponentInChildren = function(p1) end,

---函数名：GetComponentsInChildren
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---@param p3 System.Boolean
---返回值：Component[]
---@return Component[]
GetComponentsInChildren = function(p1,p2,p3) end,

---函数名：GetComponentsInChildren
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---返回值：Component[]
---@return Component[]
GetComponentsInChildren = function(p1,p2) end,

---函数名：GetComponentsInChildren
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Boolean
---返回值：T[]
---@return T[]
GetComponentsInChildren = function(p1,p2) end,

---函数名：GetComponentsInChildren
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Boolean
GetComponentsInChildren = function(p1,p2) end,

---函数名：GetComponentsInChildren
---@param p1 UnityEngine.MeshCollider
---返回值：T[]
---@return T[]
GetComponentsInChildren = function(p1) end,

---函数名：GetComponentsInChildren
---@param p1 UnityEngine.MeshCollider
GetComponentsInChildren = function(p1) end,

---函数名：GetComponentInParent
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---@param p3 System.Boolean
---返回值：Component
---@return Component
GetComponentInParent = function(p1,p2,p3) end,

---函数名：GetComponentInParent
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---返回值：Component
---@return Component
GetComponentInParent = function(p1,p2) end,

---函数名：GetComponentInParent
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Boolean
---返回值：T
---@return T
GetComponentInParent = function(p1,p2) end,

---函数名：GetComponentInParent
---@param p1 UnityEngine.MeshCollider
---返回值：T
---@return T
GetComponentInParent = function(p1) end,

---函数名：GetComponentsInParent
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---@param p3 System.Boolean
---返回值：Component[]
---@return Component[]
GetComponentsInParent = function(p1,p2,p3) end,

---函数名：GetComponentsInParent
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---返回值：Component[]
---@return Component[]
GetComponentsInParent = function(p1,p2) end,

---函数名：GetComponentsInParent
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Boolean
---返回值：T[]
---@return T[]
GetComponentsInParent = function(p1,p2) end,

---函数名：GetComponentsInParent
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Boolean
GetComponentsInParent = function(p1,p2) end,

---函数名：GetComponentsInParent
---@param p1 UnityEngine.MeshCollider
---返回值：T[]
---@return T[]
GetComponentsInParent = function(p1) end,

---函数名：GetComponents
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---返回值：Component[]
---@return Component[]
GetComponents = function(p1,p2) end,

---函数名：GetComponents
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Type
---@param p3 System.Collections.Generic.List`1[[UnityEngine.Component, UnityEngine.CoreModule, Version=0.0.0.0, Culture=neutral, PublicKeyToken=null]]
GetComponents = function(p1,p2,p3) end,

---函数名：GetComponents
---@param p1 UnityEngine.MeshCollider
GetComponents = function(p1) end,

---函数名：get_tag
---@param p1 UnityEngine.MeshCollider
---返回值：String
---@return String
tag = function(p1) end,

---函数名：set_tag
---@param p1 UnityEngine.MeshCollider
tag = function(p1) end,

---函数名：GetComponents
---@param p1 UnityEngine.MeshCollider
---返回值：T[]
---@return T[]
GetComponents = function(p1) end,

---函数名：CompareTag
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---返回值：Boolean
---@return Boolean
CompareTag = function(p1,p2) end,

---函数名：SendMessageUpwards
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---@param p3 System.Object
---@param p4 UnityEngine.SendMessageOptions
SendMessageUpwards = function(p1,p2,p3,p4) end,

---函数名：SendMessageUpwards
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---@param p3 System.Object
SendMessageUpwards = function(p1,p2,p3) end,

---函数名：SendMessageUpwards
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
SendMessageUpwards = function(p1,p2) end,

---函数名：SendMessageUpwards
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---@param p3 UnityEngine.SendMessageOptions
SendMessageUpwards = function(p1,p2,p3) end,

---函数名：SendMessage
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---@param p3 System.Object
SendMessage = function(p1,p2,p3) end,

---函数名：SendMessage
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
SendMessage = function(p1,p2) end,

---函数名：SendMessage
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---@param p3 System.Object
---@param p4 UnityEngine.SendMessageOptions
SendMessage = function(p1,p2,p3,p4) end,

---函数名：SendMessage
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---@param p3 UnityEngine.SendMessageOptions
SendMessage = function(p1,p2,p3) end,

---函数名：BroadcastMessage
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---@param p3 System.Object
---@param p4 UnityEngine.SendMessageOptions
BroadcastMessage = function(p1,p2,p3,p4) end,

---函数名：BroadcastMessage
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---@param p3 System.Object
BroadcastMessage = function(p1,p2,p3) end,

---函数名：BroadcastMessage
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
BroadcastMessage = function(p1,p2) end,

---函数名：BroadcastMessage
---@param p1 UnityEngine.MeshCollider
---@param p2 System.String
---@param p3 UnityEngine.SendMessageOptions
BroadcastMessage = function(p1,p2,p3) end,

---函数名：get_rigidbody
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
rigidbody = function(p1) end,

---函数名：get_rigidbody2D
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
rigidbody2D = function(p1) end,

---函数名：get_camera
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
camera = function(p1) end,

---函数名：get_light
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
light = function(p1) end,

---函数名：get_animation
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
animation = function(p1) end,

---函数名：get_constantForce
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
constantForce = function(p1) end,

---函数名：get_renderer
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
renderer = function(p1) end,

---函数名：get_audio
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
audio = function(p1) end,

---函数名：get_networkView
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
networkView = function(p1) end,

---函数名：get_collider
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
collider = function(p1) end,

---函数名：get_collider2D
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
collider2D = function(p1) end,

---函数名：get_hingeJoint
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
hingeJoint = function(p1) end,

---函数名：get_particleSystem
---@param p1 UnityEngine.MeshCollider
---返回值：Component
---@return Component
particleSystem = function(p1) end,

---函数名：GetInstanceID
---@param p1 UnityEngine.MeshCollider
---返回值：Int32
---@return Int32
GetInstanceID = function(p1) end,

---函数名：GetHashCode
---@param p1 UnityEngine.MeshCollider
---返回值：Int32
---@return Int32
GetHashCode = function(p1) end,

---函数名：Equals
---@param p1 UnityEngine.MeshCollider
---@param p2 System.Object
---返回值：Boolean
---@return Boolean
Equals = function(p1,p2) end,

---函数名：get_name
---@param p1 UnityEngine.MeshCollider
---返回值：String
---@return String
name = function(p1) end,

---函数名：set_name
---@param p1 UnityEngine.MeshCollider
name = function(p1) end,

---函数名：get_hideFlags
---@param p1 UnityEngine.MeshCollider
---返回值：HideFlags
---@return HideFlags
hideFlags = function(p1) end,

---函数名：set_hideFlags
---@param p1 UnityEngine.MeshCollider
hideFlags = function(p1) end,

---函数名：ToString
---@param p1 UnityEngine.MeshCollider
---返回值：String
---@return String
ToString = function(p1) end,

---函数名：GetType
---@param p1 UnityEngine.MeshCollider
---返回值：Type
---@return Type
GetType = function(p1) end,

}
